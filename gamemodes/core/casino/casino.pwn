...
Casino_Init()
{
    ...
    AddStaticPickup(19130, 1, 2633.7986, 1714.6560, 1512.4630, -1); //Casino Eingang
	CreateDynamic3DTextLabel("{FFFFFF}Visage Casino - Ausgang\nBenutze '{FF0000}ENTER{FFFFFF}' zum verlassen", 0x008080FF, 2633.7986, 1714.6560, 1512.4630, 5.0);

    AddStaticPickup(19130, 1, 2017.1334, 1916.4141, 12.3424, -1); //Casino Eingang
	CreateDynamic3DTextLabel("{FFFFFF}Visage Casino - Eingang\nBenutze '{FF0000}ENTER{FFFFFF}' zum betreten", 0x008080FF, 2017.1334, 1916.4141, 12.3424, 5.0);
    SetPlayerMapIconEx(12, 2017.1334, 1916.4141, 12.3424, 12, -1, -1, -1, -1);
    return 1;
}

Casino_OnPlayerKeyStateChange(playerid, KEY:newkeys, KEY:oldkeys )
{   
	Blackjack_OnPlayerKeyStateChange( playerid, newkeys, oldkeys );
	Roulette_OnPlayerKeyStateChange( playerid, newkeys, oldkeys );
	SlotMachines_OnPlayerKeyStateChange( playerid, newkeys, oldkeys );
    if(newkeys & KEY:16)//Entertaste
	{
	    if(IsPlayerInRangeOfPoint(playerid, 3.0, 2017.1334, 1916.4141, 12.3424))//Casino Enter
	    {
	        ...
	        SetPlayerPos(playerid,2633.7986, 1714.6560, 1512.4630);
        ...
        else if(IsPlayerInRangeOfPoint(playerid, 3.0, 2633.7986, 1714.6560, 1512.4630))//Casino Exit
	    {
	        if(IsAntiInteriorFlucht(playerid)) return SendClientMessage(playerid,COLOR_RED,"[Info]: {FFFFFF} Anti-Interiorflucht: Warte bis du das Gebäude betrittst/verlässt.");
	    	SetTimerEx("Unfreeze",5000,false,"i",playerid);
   		    TogglePlayerControllable(playerid, false);
	        SetPlayerPos(playerid,2017.1334, 1916.4141, 12.3424);
		    SetPlayerFacingAngle(playerid, 85.0523);
...
Casino_OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart)
{
