...
SlotMachines_Init( )
{
	...
	mysql_tquery( sqlHandle, "SELECT * FROM `CASINO_POOLS`", "OnCasinoPoolsLoad" );

	// load textdraws
	for ( new i = 0; i < MAX_MACHINES; i ++ )
	{
		p_SlotMachineFigureTD[ i ] = TextDrawCreate(324.000000, 307.000000, "$20,000");
		TextDrawAlignment(p_SlotMachineFigureTD[ i ], TEXT_DRAW_ALIGN_CENTER);
		TextDrawBackgroundColour(p_SlotMachineFigureTD[ i ], 255);
		TextDrawFont(p_SlotMachineFigureTD[ i ], TEXT_DRAW_FONT_3);
		TextDrawLetterSize(p_SlotMachineFigureTD[ i ], 0.250000, 1.100000);
		TextDrawColour(p_SlotMachineFigureTD[ i ], -1);
		TextDrawSetOutline(p_SlotMachineFigureTD[ i ], 1);
		TextDrawSetProportional(p_SlotMachineFigureTD[ i ], true);

		g_SlotMachineOneTD[ i ] = TextDrawCreate(222.000000, 324.000000, "ld_slot:bar1_o");
		TextDrawBackgroundColour(g_SlotMachineOneTD[ i ], 255);
		TextDrawFont(g_SlotMachineOneTD[ i ], TEXT_DRAW_FONT_SPRITE_DRAW);
		TextDrawLetterSize(g_SlotMachineOneTD[ i ], 0.500000, 0.399999);
		TextDrawColour(g_SlotMachineOneTD[ i ], -1);
		TextDrawSetOutline(g_SlotMachineOneTD[ i ], 0);
		TextDrawSetProportional(g_SlotMachineOneTD[ i ], true);
		TextDrawSetShadow(g_SlotMachineOneTD[ i ], 1);
		TextDrawUseBox(g_SlotMachineOneTD[ i ], true);
		TextDrawBoxColour(g_SlotMachineOneTD[ i ], 255);
		...
		TextDrawBackgroundColour(g_SlotMachineTwoTD[ i ], 255);
		TextDrawFont(g_SlotMachineTwoTD[ i ], TEXT_DRAW_FONT_SPRITE_DRAW);
		TextDrawLetterSize(g_SlotMachineTwoTD[ i ], 0.500000, 0.399999);
		TextDrawColour(g_SlotMachineTwoTD[ i ], -1);
		TextDrawSetOutline(g_SlotMachineTwoTD[ i ], 0);
		TextDrawSetProportional(g_SlotMachineTwoTD[ i ], true);
		TextDrawSetShadow(g_SlotMachineTwoTD[ i ], 1);
		TextDrawUseBox(g_SlotMachineTwoTD[ i ], true);
		TextDrawBoxColour(g_SlotMachineTwoTD[ i ], 255);
		...
		TextDrawBackgroundColour(g_SlotMachineThreeTD[ i ], 255);
		TextDrawFont(g_SlotMachineThreeTD[ i ], TEXT_DRAW_FONT_SPRITE_DRAW);
		TextDrawLetterSize(g_SlotMachineThreeTD[ i ], 0.500000, 0.399999);
		TextDrawColour(g_SlotMachineThreeTD[ i ], -1);
		TextDrawSetOutline(g_SlotMachineThreeTD[ i ], 0);
		TextDrawSetProportional(g_SlotMachineThreeTD[ i ], true);
		TextDrawSetShadow(g_SlotMachineThreeTD[ i ], 1);
		TextDrawUseBox(g_SlotMachineThreeTD[ i ], true);
		TextDrawBoxColour(g_SlotMachineThreeTD[ i ], 255);
		TextDrawTextSize(g_SlotMachineThreeTD[ i ], 66.000000, 77.000000);
	}

	g_SlotMachineBoxTD[ 0 ] = TextDrawCreate(220.000000, 322.000000, "_");
	TextDrawBackgroundColour(g_SlotMachineBoxTD[ 0 ], 255);
	TextDrawLetterSize(g_SlotMachineBoxTD[ 0 ], 0.500000, 7.000000);
	TextDrawUseBox(g_SlotMachineBoxTD[ 0 ], true);
	TextDrawBoxColour(g_SlotMachineBoxTD[ 0 ], 112);
	TextDrawTextSize(g_SlotMachineBoxTD[ 0 ], 430.000000, 3.000000);

	g_SlotMachineBoxTD[ 1 ] = TextDrawCreate(220.000000, 306.000000, "_");
	TextDrawBackgroundColour(g_SlotMachineBoxTD[ 1 ], 255);
	TextDrawLetterSize(g_SlotMachineBoxTD[ 1 ], 0.500000, 1.400000);
	TextDrawUseBox(g_SlotMachineBoxTD[ 1 ], true);
	TextDrawBoxColour(g_SlotMachineBoxTD[ 1 ], 238);
	TextDrawTextSize(g_SlotMachineBoxTD[ 1 ], 430.000000, -18.000000);
	return 1;
}

SlotMachines_OnPlayerKeyStateChange( playerid, KEY:newkeys, KEY:oldkeys )
{
	new
	...
	if ( IsPlayerInCasino( playerid ) )
	{
		...
		if ( machineid != -1 )
		{
			...
			if ( PRESSED( KEY_JUMP ) ) {
				...
				return SendServerMessage( playerid, "Du hast Autospin für diesen Spielautomaten %s.", p_AutoSpin{ playerid } ? ( "aktiviert" ) : ( "deaktiviert" ) );
			}
			...
			if ( PRESSED( KEY_SECONDARY_ATTACK ) )
			{
				if ( g_slotmachineData[ machineid ] [ E_ROLLING ] )
					return SendError( playerid, "Warte bis dein Zug beendet ist." );
			...
			if ( PRESSED( KEY_SECONDARY_ATTACK ) )
			{
				...
				if ( id != -1 )
				{
					...
					if ( IsPlayerInRangeOfPoint( playerid, 1.0, X, Y, g_slotmachineData[ id ] [ E_Z ] ) && GetPlayerPos( playerid, Z, Z, Z ) )
					{
						...
						return ShowPlayerHelpDialog( playerid, 0, "~y~~h~~k~~PED_SPRINT~~w~ - Spielen~n~~y~~h~~k~~PED_JUMPING~~w~ - Autospin verwalten~n~~y~~h~~k~~VEHICLE_ENTER_EXIT~~w~ - Verlassen" );
					}
				}
			}
		}
...
SlotMachines_OnPlayerDisconnect( playerid )
{
	p_usingSlotMachine[ playerid ] = -1;
	return 1;
}

SlotMachines_OnPlayerDeath( playerid )
{
...
public OnPlayerUseSlotMachine( playerid, slotid, first_combo, second_combo, third_combo )
{
	...
	if ( ! Iter_Contains( CasinoPool, poolid ) )
		return SendError( playerid, "Fehler beim Spielautomaten! (0xFF33)" );
	...
	if ( first_combo == second_combo && first_combo == third_combo )
	{
		...
		if ( iNetWin > g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] ) {
			...
			foreach ( new i : Player ) {
				SendClientMessage( i, -1, ""COL_GREY"[CASINO]{FFFFFF} %s(%d) hat "COL_GOLD"%s"COL_WHITE" im Casino %s am Spielautomaten gewonnen!", ReturnPlayerName( playerid ), playerid, cash_format( iNetWin ), g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] == 10000 ? ( "Four Dragons" ) : ( g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] >= 25000 ? ( "Visage" ) : ( "Caligulas" ) ) );
			}
		} else {
   			SendServerMessage( playerid, "Glückwunsch, du hast "COL_GOLD"%s"COL_WHITE" gewonnen!", cash_format( iNetWin ) );
   		}

		// give the cash
		GivePlayerMoneyEx( playerid, iNetWin );
		PlayerPlaySound( playerid, 4201, 0.0, 0.0, 0.0 ); // Coin fall
		GameTextForPlayer( playerid, "~w~~h~Gewinner!", 5000, 6 );
   		return 1;
	}

	return GameTextForPlayer( playerid, "~w~~h~Kein Gewinn!", 2500, 6 );
}

/* ** SQL Threads ** */
forward OnCasinoPoolsLoad( );
public OnCasinoPoolsLoad( )
{
	new
	    rows;
    ...
    if ( rows )
    {
    	for( new i = 0; i < rows; i++ )
    	{
	    	new
	    		poolid;
            cache_get_value_name_int(i, "ID", poolid);
	    	...
	    	if ( 0 <= poolid < MAX_SLOT_POOLS )
	    	{
	    		...
	    		// insert data
	    		cache_get_value_name_int( i, "POOL", g_casinoPoolData[ poolid ] [ E_POOL ] );
	    		cache_get_value_name_int( i, "TOTAL_WINNINGS", g_casinoPoolData[ poolid ] [ E_TOTAL_WINNINGS ] );
	    		cache_get_value_name_int( i, "TOTAL_GAMBLED", g_casinoPoolData[ poolid ] [ E_TOTAL_GAMBLED ] );
		...
		mysql_tquery( sqlHandle, "SELECT * FROM `SLOT_MACHINES`", "OnSlotMachinesLoad" );
    }
    return 1;
}

forward OnSlotMachinesLoad( );
public OnSlotMachinesLoad( )
{
	new
	    rows, loadingTick = GetTickCount( );
    ...
    if ( rows )
    {
    	for( new i = 0; i < rows; i++ )
    	{
			...
			if ( id != ITER_NONE )
			{
                ...
                cache_get_value_name_float(i, "ROTATION", rZ);
				...
				cache_get_value_name_int( i, "ENTRY_FEE", g_slotmachineData[ id ] [ E_ENTRY_FEE ] );
				cache_get_value_name_int( i, "POOL_ID", g_slotmachineData[ id ] [ E_POOL_ID ] );

				// 3d Text
				fOffsetX = 1.0 * floatsin( -rZ, degrees );
				fOffsetY = 1.0 * floatcos( -rZ, degrees );
				CreateDynamic3DTextLabel( sprintf( "Drücke ENTER um zu spielen\n"COL_WHITE"%s Min.", cash_format( g_slotmachineData[ id ] [ E_ENTRY_FEE ] ) ), COLOR_GREY, X + fOffsetX, Y + fOffsetY, Z - 0.1, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, .testlos = 1 );
...
stock TriggerPlayerSlotMachine( playerid, machineid )
{
	...
	if ( !g_slotmachineData[ machineid ] [ E_ROLLING ] )
	{
		...
		if ( GetPlayerMoneyEx( playerid ) < entryFee )
			return SendError( playerid, "Du benötigst mind. %s um an diesem Automaten zu spielen.", cash_format( entryFee ) ), ( p_AutoSpin{ playerid } = false ), 1;
		...
		ApplyAnimation( playerid, "CASINO", "slot_plyr", 2.0, false, true, true, false, 0 );
		GivePlayerMoneyEx( playerid, -entryFee );
		return 1;
	}
	return 1;
}
...
stock UpdateCasinoPoolLabels( poolid )
{
	for( new i = 0; i < POOL_ENTITIES; i ++ )
	{
		if ( IsValidDynamicObject( g_casinoPoolData[ poolid ] [ E_OBJECT ] [ i ] ) )
			SetDynamicObjectMaterialText( g_casinoPoolData[ poolid ] [ E_OBJECT ] [ i ], 0, sprintf( "%s Preis", cash_format( g_casinoPoolData[ poolid ] [ E_POOL ] ) ), 130, "Arial", 20, 1, 0xFF00FF00, 0, 1 );

		if ( IsValidDynamic3DTextLabel( g_casinoPoolData[ poolid ] [ E_LABEL ] [ i ] ) )
			UpdateDynamic3DTextLabelText( g_casinoPoolData[ poolid ] [ E_LABEL ] [ i ], 0x00FF00FF, sprintf( "%s Preis Jackpot", cash_format( g_casinoPoolData[ poolid ] [ E_POOL ] ) ) );
	}
	return 1;
}
...
stock UpdateCasinoPoolData( poolid, pool_increment = 0, total_win = 0, total_gambled = 0 )
{
	...
	if ( gettime( ) > iUpdateCooldown )
	{
		...
		mysql_tquery( sqlHandle, szNormalString );

		// cooldown
		iUpdateCooldown = gettime( ) + 20;
	}
}
...
forward rollMachine( playerid, id, Float: velocity, spins );
public rollMachine( playerid, id, Float: velocity, spins )
{
	new
