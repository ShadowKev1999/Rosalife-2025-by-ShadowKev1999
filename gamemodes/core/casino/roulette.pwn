		...
		if ( p_RouletteMarkerTimer[ playerid ] != -1 )
		{
			...
			if ( PRESSED( KEY_SPRINT ) )
			{
				...
				if ( g_rouletteTableData[ rouletteid ] [ E_SPINNING_TIMER ] == -1 )
				{
					...
					SendServerMessage( playerid, "Du hast %s gesetzt. Viel Glück!", cash_format( totalBet ) );
				}
				else SendServerMessage( playerid, "Du bist der Runde beigetreten, viel Glück!" );
			}
			// decrease bet
			if ( PRESSED( KEY_WALK ) )
			{
		...
		if ( PRESSED( KEY_SECONDARY_ATTACK ) )
		{
			...
			foreach(new id : roulettetables) if ( GetPlayerVirtualWorld( playerid ) == g_rouletteTableData[ id ] [ E_WORLD ] )
			{
				if ( IsPlayerInRangeOfPoint( playerid, 3.1, g_rouletteTableData[ id ] [ E_X ], g_rouletteTableData[ id ] [ E_Y ], g_rouletteTableData[ id ] [ E_Z ] ) ) {
					...
					SendServerMessage( playerid, "Dein Startbudget ist $1,000 für diesen Tisch. {%06x}Das ist unsere Chip-Farbe.", g_rouletteChipColor[ playerid ] >>> 8 );
...
public OnRouletteWheelStop( rouletteid, winner )
{
	...
	foreach(new playerid : Player) if ( p_RouletteTable[ playerid ] == rouletteid && p_rouletteBetLocked{ playerid } )
	{
		...
		if ( profit > 0 )  {
			...
			SendClientMessage( playerid, -1, ""COL_GREY"[ROULETTE]"COL_WHITE" Du hast %s gesetzt und erhälst %s! (Gewinner No %d)", cash_format( waged ), cash_format( profit ), winner );
		} else {
