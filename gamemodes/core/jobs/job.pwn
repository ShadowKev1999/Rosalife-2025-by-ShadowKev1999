...
stock DeleteJobFahrzeug(playerid)
{
	...
	else if(GetPVarInt(playerid,"MùllmannArbeitet") != 0)
	{
	    KillTimer(MuellmannTimer[playerid]);
  		new tmpcar = GetPVarInt(playerid,"MùllAuto");
	    UpdateDynamic3DTextLabelText(FahrzeugInfo[tmpcar][fLabel], COLOR_WHITE, "Mùllmann:\n{FE0000}Niemand{FFFFFF}\nMùll: {FF0000}0{FFFFFF} Liter");
	    SetVehicleToRespawn(tmpcar);
	    FahrzeugInfo[tmpcar][fJobFahrer] = 0;
	    DeletePVar(playerid,"MùllBeutel");
	    DeletePVar(playerid,"MùllBeutelHand");
	    DeletePVar(playerid,"MùllmannArbeitet");
	...
	else if(GetPVarInt(playerid,"ZugFahrerVehicle") != INVALID_VEHICLE_ID)
	{
		...
		//Lùschen der Zugmaschine und der Trailer
        ...
        //Zurùcksetzen der Trailer-Variablen und Lùschung der Trailer-Objekte
...
stock IsJobDuty(playerid)
{
	...
	GetPVarInt(playerid,"BusfahrerDuty") == 0 && GetPVarInt(playerid,"MùllmannDuty") == 0 &&
