@echo off
chcp 65001 >nul
title Rosalife-2025 Compiler Fix

echo.
echo ========================================
echo    Rosalife-2025 Compiler Fix
echo ========================================
echo.

REM Überprüfen ob pawno existiert
if not exist "pawno\pawno.exe" (
    echo [FEHLER] pawno.exe nicht gefunden!
    echo Bitte stellen Sie sicher, dass sich pawno.exe im pawno\ Ordner befindet.
    pause
    exit /b 1
)

echo [INFO] Pawno gefunden
echo.
...
echo [INFO] Starte optimierten Compiler...
echo.
...
pawno\pawno.exe -t300 -r gamemodes\skg.pwn
...
:end
echo.
echo [INFO] Compiler-Fix abgeschlossen
pause
